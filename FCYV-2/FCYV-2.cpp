...
static int                V = 0;            /* número de vértices               */
...
/* ── Parser de enteros sin signo sobre un buffer mmap ─────────────────────── */
static inline uint32_t parse_uint(const char *__restrict__ &p, const char *end)
{
    while (p < end && (*p < '0' || *p > '9')) ++p;   /* saltar no-dígitos */
    uint32_t x = 0;
    while (p < end && *p >= '0' && *p <= '9') {
...
static void read_graph(const char *filename)
{
    ...
    /* Pasada 1: contar grados de salida (saltando self-loops). */
    int64_t *outdeg = adj_start + 1;        /* scratch en adj_start[1..V] */
    uint32_t valid = 0;
    for (uint32_t i = 0; i < M && p < end; ++i) {
        uint32_t src = parse_uint(p, end);
        uint32_t dst = parse_uint(p, end);
        parse_uint(p, end);                 /* type (descartado en pasada 1) */
        if (src == dst) continue;
        ++outdeg[src - 1];
        ++valid;
    }
    ...
    /* Pasada 2: colocar aristas empaquetadas (dest<<2)|type. */
...
static inline void search_motif(int hub, uint32_t g)
{
    ...
    /* Sucesores del hub: solo los de id > hub (dedup estático). */
    for (int64_t k = b; k < e; ++k) {
        const uint32_t edge = adj[k];
        const int dst = (int)(edge >> 2);
        if (dst <= hub) continue;           /* "ya fue hub" <=> dst <= hub */
    ...
    for (int qi = 0; qi < qn; ++qi) {
        ...
        for (int64_t k = sb; k < se; ++k) {
            ...
            if (n <= hub) continue;         /* "ya fue hub" <=> n <= hub */
            const NodeState &x = ns[n];
            if (x.iter == g) continue;      /* ya procesado como s en este hub */
            const int tn = (int)(edge & 3u);
            ++nexpansions;
            if (x.hub_gen == g) {
                /* n es sucesor directo del hub (mismo parent) -> triángulo. */
                const int nt = x.tpc;
                int ta = ts, tb = nt;
...
int main(int argc, char **argv)
{
    ...
    const double secs = (t1.tv_sec - t0.tv_sec) + (t1.tv_nsec - t0.tv_nsec) * 1e-9;
