/*
 * FC3R-00: variante de FC3R (EfficientGraph.c) que reemplaza la lista
 * enlazada (arrays toEdge/nextEdge recorridos via nodes[u].head -> nextEdge[e])
 * por un CSR real (adj_start[] con offsets, toEdge[]/toEdgeType[] contiguos
 * por nodo). Es el UNICO cambio: se mantiene el mismo orden de hubs (qsort
 * por grado descendente sobre order[]), el mismo dedup dinamico (color/iter),
 * la misma lectura por fscanf, los mismos prints verbosos y el mismo timer.
 * El codigo reemplazado queda comentado, no borrado.
 */
